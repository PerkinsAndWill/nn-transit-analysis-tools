...
folder_name = "Bakersfield"

#Coordinate systems
coord_global = 4326
coord_local = 2229 #see here: https://nelsonnygaard.shinyapps.io/coord-system-reference/

#Path to GTFS feed you will use
feed_path = "/GET_20230202.zip"

#A character string to identify the time period of the GTFS feed you are using
app_time_period = "Winter 2023"
...
weekday_transit_supply_geom <- sub_block_groups %>%
...
primary_shape_ref <- shape_pattern_ref %>%
  ...
  filter(shape_id %in% stop_dirs$shape_id) %>%
...
write_rds(sub_route_ref,paste0(full_output_folder,"/bakersfield_route_reference.rds"))
...
################################Do not Run###########################################################
###############################################Error will occur if add non bus routes and the system have no non bus
...
htmlwidgets::saveWidget(map_obj,file = "docs/transit-supply-example-maps/trimet-example-bg.html")
...
hex_radius = 5280*0.25

hex_grid <- sub_us_counties %>%
  st_union() %>%
  st_transform(coord_local) %>%
  st_make_grid(cellsize = hex_radius*2, square = FALSE) %>%
  st_as_sf() %>%
  filter(st_intersects(.,sub_us_counties %>% st_union() %>% st_transform(coord_local),
                       sparse = FALSE)) %>%
  rename(geometry = x) %>%
  mutate(hex_id = seq_along(geometry)) %>%
  select(hex_id,geometry)

# leaflet() %>%
#   addProviderTiles("CartoDB.Positron") %>%
#   addPolygons(data = hex_grid %>% st_transform(coord_global)) %>%
#   addMeasure()

## Calculating Transit Supply ----------------

#Calculate transit supply metric for each hex bin
with_progress({
  
  p <- progressor(steps = nrow(hex_grid))
  
  hex_bins_with_transit_supply <- hex_grid %>%
    mutate(transit_supply_raw = future_pmap(
      .l = list(geometry),
      .f = function(geom, p){
        ...
        #hex geometry
        hex <- geom %>% st_sfc(crs=coord_local)
        
        #Walksheds intersecting with given block group
        sub_walksheds <- stop_walksheds %>%
          filter(st_intersects(.,hex,sparse = FALSE)) 
        ...
        if(nrow(sub_walksheds)>0){
          ...
          #Whole area of hexagon (in acres)
          whole_hex_area <- as.numeric(st_area(hex))*2.29568e-5
          ...
          transit_supply_raw_calc <- sub_route_trips %>%
            ...
            mutate(transit_supply_raw = value * stop_area_acres) %>%
            group_by(day_cat) %>%
            summarise(transit_supply_raw = sum(transit_supply_raw)/whole_hex_area) 
...
transit_supply_results <- hex_bins_with_transit_supply %>%
  st_drop_geometry() %>%
...
htmlwidgets::saveWidget(map_obj,file =paste0(full_output_folder, "/Bakersfield-hex.html"))

#Save the hexagon shapefile to r object
saveRDS(transit_supply_geom, paste0(full_output_folder, "/TSI_hex.rds"))

#Save the hexagon to shapefile

transit_supply_geom = transit_supply_geom %>% 
  mutate(transit_supply_raw = round(transit_supply_raw ,3),
         transit_supply_score = round(transit_supply_score, 3)) %>%
  as_Spatial()

writeOGR(transit_supply_geom, dsn = full_output_folder, layer = 'transit_supply_hexagon', driver = "ESRI Shapefile")
